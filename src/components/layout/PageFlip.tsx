...
/** The element whose contents are the current page (the routed content inside the shell). */
 ...
 * cleanup runs while its DOM is still attached), then re-drawn on a sheet of paper in a fixed layer over
 * the content area and swung about its left edge — split into strips on nested hinges so the sheet bends
 * like paper instead of turning like cardboard. Going deeper into the app the old page flips away and the
 * new page is already underneath; going back, the new page flips in from the left and lands on top of a
 * still copy of the old one. Jumping several sections riffles a blank sheet or two in between.
 *
 * The hinge is the content area's left edge: beside the sidebar on desktop, the screen edge on a phone.
 */
...
interface Snap {
  clone: HTMLElement
  /** where the sheet sits on screen (the visible part of the content area) */
  rect: { left: number; top: number; width: number; height: number }
  /** how far the copy has to be shifted up so it lines up with what was on screen */
  offsetTop: number
}

/** The visible part of the content area — the sheet that turns. */
function pageFrame(root: HTMLElement) {
  const r = root.getBoundingClientRect()
  const top = Math.max(0, r.top)
  let floor = window.innerHeight
  // the phone's bottom bar sits over the content; the sheet stops above it rather than turning through it
  const bar = document.querySelector('nav[aria-label="Primary"]')
  if (bar) {
    const b = bar.getBoundingClientRect()
    if (b.height > 0 && b.top < floor && getComputedStyle(bar).position === 'fixed') floor = b.top
  }
  const bottom = Math.min(floor, Math.max(r.bottom, top + 1))
  return { rect: { left: r.left, top, width: r.width, height: Math.max(1, bottom - top) }, offsetTop: r.top - top }
}
 ...
 */
function snapshotPage(root: HTMLElement): Snap | null {
  const { rect, offsetTop } = pageFrame(root)
  const clone = pruneClone(root, rect.top - 60, rect.top + rect.height + 60, 0)
  clone.removeAttribute('id')
  clone.style.cssText = 'position:relative;margin:0;padding:0;min-height:0;width:100%;'
  return { clone, rect, offsetTop }
}
...
function runFlip(opts: { rect: Snap['rect']; forward: boolean; sheets: Sheet[]; still: Snap | null; strips: number }): FlipJob {
  const { rect, forward, sheets, still } = opts
  // The hinge is the left edge ('y'); 'x' (a top hinge) is kept for a flip-pad layout.
  const axis = ((): 'x' | 'y' => 'y')()
  const N = Math.max(2, opts.strips)
  const layer = div('flip-layer')
  layer.setAttribute('aria-hidden', 'true')
  const box = (r: Snap['rect']) => `left:${r.left}px;top:${r.top}px;width:${r.width}px;height:${r.height}px;`
  const at = box(rect)
  const W = rect.width
  const H = rect.height
  ...
  if (still) {
    const base = div('flip-sheet', box(still.rect))
    const paper = div('flip-paper', `width:${still.rect.width}px;height:${still.rect.height}px;`)
    const c = still.clone.cloneNode(true) as HTMLElement
    c.style.marginTop = `${still.offsetTop}px`
    paper.appendChild(c)
  ...
  const D = (window.innerWidth < 640 ? 760 : 900) * slow
  ...
  for (const k of drawOrder) {
    ...
    const el = div('flip-sheet', sheet.snap ? box(sheet.snap.rect) : at)
    const SW = sheet.snap ? sheet.snap.rect.width : W
    const SH = sheet.snap ? sheet.snap.rect.height : H
    el.style.perspective = `${Math.max(2400, (axis === 'y' ? SW : SH) * 3.2)}px`
    el.style.perspectiveOrigin = axis === 'y' ? '4% 50%' : '50% 4%'
    const stripW = axis === 'y' ? Math.ceil(SW / N) : SW
    const stripH = axis === 'y' ? SH : Math.ceil(SH / N)
    ...
    for (let i = 0; i < N; i++) {
      ...
      const paper = div('flip-paper', `width:${SW}px;height:${SH}px;${axis === 'y' ? `left:${-i * stripW}px` : `top:${-i * stripH}px`};`)
      if (sheet.snap) {
        ...
        c.style.marginTop = `${sheet.snap.offsetTop}px`
        paper.appendChild(c)
      } else {
...
export function PageFlip({ children }: { children: ReactNode }) {
  ...
  const onLeave = useCallback((path: string) => {
    ...
    leaving.current = { path, snap: root ? snapshotPage(root) : null }
  }, [])
  ...
  useLayoutEffect(() => {
    ...
    const rect = pageFrame(root).rect
    ...
    if (forward) {
      ...
      const incoming = snapshotPage(root)
      still = left.snap
      sheets = [...Array.from({ length: jump - 1 }, () => ({ snap: null })), { snap: incoming }]
    }
    current = runFlip({ rect, forward, sheets, still, strips })

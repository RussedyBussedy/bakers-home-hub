...
type Filter = 'all' | 'live' | ProjectStatus
...
export default function ProjectsPage() {
  ...
  const list = useMemo(() => {
    ...
    return data.projects
      .filter((p) => (filter === 'all' ? true : filter === 'live' ? p.status === 'in_progress' || p.status === 'planning' : p.status === filter))
  ...
  const counts = useMemo(() => {
    const c: Record<string, number> = { all: data.projects.length, live: data.projects.filter((p) => p.status === 'in_progress' || p.status === 'planning').length }

...
2. Open **SQL Editor → New query**, paste the whole of `supabase/schema.sql`, click **Run**. This creates every table, the row-level security that locks data to your household, the private `media` bucket, and the live-sync publication.
   Then run each file in `supabase/migrations/` in order the same way (they're additive — `002` nudges and WhatsApp numbers, `003` deposits on quotes, `004` site days and blockers, `005` a new account gets its own household, `006` invites, `007` naming your own home, `008` currency, `009` the house lists and meters, `010` meter dials, `011` The Word, `012` hidden letters and the PIN, `013` the study threads, `014` clock times on meter readings and top-ups, `015` study per reading).
...
- A household is named by its people at registration and renameable in Settings. It shows as "<name> Hub" — `homeTitle()` in `src/lib/utils.ts` only adds the word when the name doesn't already end in one like it, so "The Bakers" reads "The Bakers Hub" while "Gran's Home" is left alone (`npm run hometitle-test`).
- Settings lives in the sidebar on a desktop and behind the faces in the Hub header on a phone (there is no sidebar there) — `flows.mjs` checks it stays reachable at phone width.
...
- `src/components/house/WordPanel.tsx` + `supabase/functions/guide/` – **The Word**, the fourth tab of the House: write down what is going on in your life and get a pastor's letter back — the passages that speak to it (quoted from the Hub's own Bible, never from the model's memory), what they mean for you, a few things to do this week, a prayer, and a week of readings you can open and tick off in place. Letters are private to the person who asked (`public.bible_guidance`, row-level security by user). A letter can also be **hidden** behind a 4–8 digit PIN (`012_hidden_letters.sql`: `bible_prefs` keeps a bcrypt hash, five wrong guesses lock it for 15 minutes; hidden rows are invisible to the ordinary policies and only come back through the `bible_hidden_*` functions, which check the PIN server-side), either after the fact or at the moment of writing ("Keep this one hidden"). Face ID / Touch ID / fingerprint is a per-device convenience on top (`src/lib/biometric.ts`: a platform passkey guards a copy of the PIN in that browser's storage; the database still only ever sees the PIN). `scripts/local/pin-rls-test.mjs` proves the policies and PIN functions against a local Postgres. Under every letter is a **Study** thread (`013_study.sql`, `public.bible_study`): three starter questions come with the letter, and any question — a verse, a person, a word — goes back through `guide` (action `study`) with the letter and the thread so far as context; the answer quotes only from the index, the references it mentions are checked against the canon so they open in place, and a hidden letter's thread is reachable only through the PIN functions. Each reading of the plan has a study of its own as well (`015_study_readings.sql`: `reading_index`, 0 = Day 1, null = the letter): the reading's text is fetched and offered to the model as source `r`, quotable by verse range, only that reading's thread is context, each reading comes with one starter question, and a question can be moved between the letter and its readings — the one column a person may update. Any verse, step, prayer or answer can be sent on WhatsApp, shared or copied (`ShareButton`). Nothing a person sees names the model behind the letters — keep it that way. The Bible itself — Berean Standard Bible and King James text, Nave's Topical Bible, the OpenBible cross-references and their embeddings — lives in the `bible` schema, loaded and embedded by the separate [BibleBot](https://github.com/RussedyBussedy/BibleBot) repo's *Load and embed* workflow. To switch it on: run that workflow, run `supabase/migrations/011_the_word.sql`, deploy `guide` the same way as `unfurl` (paste `index.ts`), and add the secret `GEMINI_API_KEY` (optionally `GEMINI_MODEL`). If someone writes about being in danger or wanting to end their life, the letter opens with South African helplines whatever the model made of it. `npm run guide-test` checks the guard rails (references, candidate passages, the safety screen); `npm run guide-flow-test` runs the function end to end against stubbed services.

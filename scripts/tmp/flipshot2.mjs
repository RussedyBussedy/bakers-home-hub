...
const times = (process.argv[5] || '1200,1800,2400,3000,3600,4200').split(',').map(Number)
...
for (const at of times) {
  ...
  await page.screenshot({ path:`/tmp/flip2-${mode}-${at}.png` })
}
await page.waitForTimeout(8000)
console.log('stale layers:', await page.evaluate(() => document.querySelectorAll('.flip-layer').length))
await browser.close()

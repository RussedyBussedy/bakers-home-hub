...
await step('the Word: hiding a letter asks for a PIN first, then it is gone from the list', async () => {
  await page.setViewportSize({ width: 1280, height: 860 })
  await page.goto(base + '/house?tab=word', { waitUntil: 'networkidle' })
  await page.getByText('From the Word').waitFor()
  await page.getByRole('button', { name: 'Hide' }).click()
  await page.getByTestId('pin-form').waitFor()
  await page.getByLabel('PIN', { exact: true }).fill('2468')
  await page.getByLabel('Again, to be sure').fill('2468')
  await page.getByRole('button', { name: 'Set PIN' }).click()
  await page.getByText('1 hidden').waitFor({ timeout: 5000 })
  // It was the only letter, so the composer is back and the letter is nowhere on the page.
  await page.getByLabel('What’s on your heart?').waitFor()
  if (await page.getByText('From the Word').count()) throw new Error('the hidden letter is still on screen')
  if (await page.getByText(/Someone I love has died/).count()) throw new Error('the hidden letter is still listed')
  await shot('27-word-hidden')
})

await step('the Word: a wrong PIN is refused, the right one opens it, Lock shuts it again', async () => {
  await page.getByRole('button', { name: 'Unlock hidden letters' }).click()
  await page.getByTestId('unlock-form').waitFor()
  await page.getByLabel('PIN', { exact: true }).fill('0000')
  await page.getByRole('button', { name: 'Unlock', exact: true }).click()
  await page.getByText(/That isn’t it — 4 tries left/).waitFor({ timeout: 5000 })
  await page.getByLabel('PIN', { exact: true }).fill('2468')
  await page.getByRole('button', { name: 'Unlock', exact: true }).click()
  await page.getByText('Hidden · 1').waitFor({ timeout: 5000 })
  ...
  await page.waitForTimeout(300)
  if (await page.getByText('From the Word').count()) throw new Error('the hidden letter stayed open after locking')
})

await step('the Word: a letter can be born hidden', async () => {
  await page.getByLabel('What’s on your heart?').fill('I made a mess of things at work and I do not know how to face Monday morning.')
  await page.getByRole('checkbox', { name: 'Keep this one hidden' }).click()
  await page.getByRole('button', { name: 'Ask for a word' }).click()
  await page.getByText('From the Word').waitFor({ timeout: 15000 })
  if (!(await page.getByRole('button', { name: 'Unhide' }).count())) throw new Error('the new letter is not marked hidden')
  await page.getByText('2 hidden').waitFor({ timeout: 5000 })
  if (await page.getByText('Nothing yet').count() === 0) throw new Error('a born-hidden letter must not appear in the open list')
  await shot('29-word-born-hidden')
})

await step('the Word: fingerprint unlocks without the PIN (virtual authenticator)', async () => {
  // WebAuthn needs a real host name, so this one runs on localhost — a fresh origin with fresh demo state.
  const cdp = await context.newCDPSession(page)
  await cdp.send('WebAuthn.enable')
  await cdp.send('WebAuthn.addVirtualAuthenticator', { options: { protocol: 'ctap2', transport: 'internal', hasResidentKey: true, hasUserVerification: true, isUserVerified: true, automaticPresenceSimulation: true } })
  const local = base.replace('127.0.0.1', 'localhost')
  await page.goto(local + '/house?tab=word', { waitUntil: 'networkidle' })
  await page.getByLabel('What’s on your heart?').fill('I am worn out and running on empty, and I cannot remember the last time I rested.')
  await page.getByRole('checkbox', { name: 'Keep this one hidden' }).click()
  await page.getByTestId('pin-form').waitFor()
  await page.getByLabel('PIN', { exact: true }).fill('1357')
  await page.getByLabel('Again, to be sure').fill('1357')
  await page.getByRole('button', { name: 'Set PIN' }).click()
  await page.getByTestId('pin-form').waitFor({ state: 'hidden' })
  await page.getByRole('button', { name: 'Ask for a word' }).click()
  await page.getByText('From the Word').waitFor({ timeout: 15000 })
  await page.getByText('1 hidden').waitFor()
  // Unlock with the PIN once, switch the fingerprint on, lock…
  await page.getByRole('button', { name: 'Unlock hidden letters' }).click()
  await page.getByLabel('PIN', { exact: true }).fill('1357')
  await page.getByRole('button', { name: 'Unlock', exact: true }).click()
  await page.getByText('Hidden · 1').waitFor({ timeout: 5000 })
  await page.getByRole('button', { name: 'Hidden letters settings' }).click()
  await page.getByRole('menuitem', { name: /Use fingerprint or face unlock on this device/ }).click()
  await page.getByText(/switched on/).waitFor({ timeout: 5000 })
  await page.getByRole('button', { name: 'Lock hidden letters' }).click()
  await page.getByRole('button', { name: 'Unlock hidden letters' }).waitFor()
  // …and now the prompt (answered by the virtual authenticator) opens it with no PIN typed.
  await page.getByRole('button', { name: 'Unlock hidden letters' }).click()
  await page.getByText('Hidden · 1').waitFor({ timeout: 5000 })
  if (await page.getByTestId('unlock-form').count()) throw new Error('the PIN sheet opened even though the fingerprint was enrolled')
  await shot('30-word-fingerprint')
  // Switching it off puts the PIN sheet back.
  await page.getByRole('button', { name: 'Hidden letters settings' }).click()
  await page.getByRole('menuitem', { name: /Stop using fingerprint or face unlock here/ }).click()
  await page.getByRole('button', { name: 'Lock hidden letters' }).click()
  await page.getByRole('button', { name: 'Unlock hidden letters' }).click()
  await page.getByTestId('unlock-form').waitFor({ timeout: 5000 })
  await page.keyboard.press('Escape')
  await cdp.detach()
  await page.goto(base + '/house?tab=word', { waitUntil: 'networkidle' })
})
